...
//base by CREPUS XD (CREPUS XD)
//YouTube: @esteams
//WhatsApp: CREPUS CHANNEL 
//Telegram: t.me/Zainz_channel 
//GitHub: @paskito002
//WhatsApp: +2250566593696 / +50956935152
...
let phoneNumber = process.env.CREPUS_PHONE || process.env.STINGER_PHONE || ""
...
const sessionDir = process.env.CREPUS_SESSION_DIR || process.env.STINGER_SESSION_DIR || `./session`
const { state, saveCreds } = await useMultiFileAuthState(sessionDir)
   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (phoneNumber) {
         inputPhoneNumber = phoneNumber.replace(/[^0-9]/g, '')

         if (!Object.keys(PHONENUMBER_MCC).some(v => inputPhoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +2349155298855")))
            process.exit(0)
         }
      } else {
         inputPhoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Es Teams said you should Please type your WhatsApp number 🥺\nFor example: +2250566593696 / +50956935152 : `)))
      ...
      console.log(`[PAIRING] Preparing pairing code request for ${inputPhoneNumber}`)
      setTimeout(() => {
         Promise.resolve()
           .then(async () => {
             console.log(`[PAIRING] Calling requestPairingCode for ${inputPhoneNumber}`)
             let code = await XeonBotInc.requestPairingCode(inputPhoneNumber)
             code = code?.match(/.{1,4}/g)?.join("-") || code
             console.log(`[PAIRING] requestPairingCode returned successfully`)
             console.log(chalk.black(chalk.bgGreen(`Your Pairing Code : `)), chalk.black(chalk.white(code)))
           })
           .catch((err) => {
             console.error(`[PAIRING_ERROR] requestPairingCode failed: ${err?.stack || err}`)
             process.exitCode = 1
           })
      }, 3000)
   }
    ...
    XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require("./CREPUS-XD")(XeonBotInc, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
	        ...
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: es tesms`))
...
startXeonBotInc().catch((err) => {
    console.error(`[FATAL] startXeonBotInc failed: ${err?.stack || err}`)
    process.exitCode = 1
})
